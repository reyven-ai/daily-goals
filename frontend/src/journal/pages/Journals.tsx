  ...
  return (
    <div className="border-r border-[1px] w-[300px] h-[100vh] overflow-auto">
      ...
      {Object.keys(filteredGroupedJournals).length === 0 ? (
        <p className="text-center text-[15px] mt-[3rem] text-black text-opacity-65">No journals yet</p>
      ) : (
        Object.entries(filteredGroupedJournals).map(([groupName, journals]) => (
          <div key={groupName}>
            <h2 className="text-[13px] border-b p-2 text-[#7d7d7d] text-opacity-100 font-semibold mt-3">{groupName}</h2>
            <ul>
              {journals.map((journal) => (
                <Link to={journal.id} className="block" key={journal.id}>
                  <li
                    className={cn('p-6', 'border-b', selectedId === journal.id ? 'bg-[#ececec]' : '')}
                  ...
                  >
                    ...
                    <p className="text-[13px] text-black text-opacity-65">{formatDateJournal(journal.updatedAt)}</p>
                  </li>
                </Link>
              ))}
            </ul>
